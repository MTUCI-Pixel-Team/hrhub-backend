...
class Command(BaseCommand):
    ...
    def is_valid_phone_number(self, phone_number):
        ...
        if bool(re.match(pattern, phone_number)) and len(phone_number) > 7 and len(phone_number) > 10:
            return phone_number
        else:
            return 0
    ...
    def run_bot(self):
        ...
        for event in longpoll.listen():
            if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                ...
                elif self.user_states.get(user_id) == "choose_hr" and message == "вперед":
                    self.page += 1
                    self.send_hr_list(vk, user_id, self.page)
                elif self.user_states.get(user_id) == "choose_hr" and message == "назад":
                    self.page -= 1
                    self.send_hr_list(vk, user_id, self.page)
    ...
    def send_help_message(self, vk, user_id):
        ...
        vk.messages.send(
            user_id=user_id,
            message=("Этот бот предназначен для связи с HR,  зарегистрировавшимися на сайте HRhub. Для начала работы нажмите Старт, далее нажмите кнопку 'Выбрать HR'\n"
